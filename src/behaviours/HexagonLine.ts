...
export class HexagonLine extends Behaviour {
  ...
  static reDrawBorderLine() {
    ...
    for(let i=1;i<=Nation.nationQuantity;i++){
      ...
      if (newListLength[i] != HexagonLine.oldlistlength[i]) {
        ...
        HexagonLine.caculateOwnerProvinces(i)
        HexagonLine.judgeSameLine(i)
      }
    }
  }

  static caculateOwnerProvinces(nationId: number) {
  ...
  // 计算六边形的顶点坐标
  static caculateVertices(centerX: number, centerY: number, radius: number, nationId: number) {
    for (let i = 0; i < 6; i++) {
  ...
  static judgeSameLine(nationId: number) {
  ...
  static judgeSameVertex(vertex: { x: number, y: number }, vertex2: { x: number, y: number }): boolean {
