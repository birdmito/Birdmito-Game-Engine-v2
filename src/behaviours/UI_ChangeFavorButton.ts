...
export class UI_ChangeFavorButton extends Behaviour {
    ...
    onUpdate(): void {
        // 获取玩家对该国家的外交政策
        switch (Nation.nations[GameProcessMgr.playerNationId].foreignPolicy.get(this.nation.nationId)) {
            case 'positive':
        ...
        this.gameObject.onMouseLeftDown = () => {
            switch (Nation.nations[GameProcessMgr.playerNationId].foreignPolicy.get(this.nation.nationId)) {
                case 'positive':
                    Nation.nations[GameProcessMgr.playerNationId].changeForeignPolicy(this.nation, 'neutral')
                    break;
                case 'neutral':
                    Nation.nations[GameProcessMgr.playerNationId].changeForeignPolicy(this.nation, 'negative')
                    break;
                case 'negative':
                    Nation.nations[GameProcessMgr.playerNationId].changeForeignPolicy(this.nation, 'positive')
                    break;
            }
        }
    }
